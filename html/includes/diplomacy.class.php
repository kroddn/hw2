...
class diplomacy {
    ...
    if (($data1 = do_mysql_fetch_assoc($res1)) &&
        ...
        ($data2['clan'] > 0))    
        {
          if (($data1['clan'] == $data2['clan']) && $type == 0) {
            ...
            while($data7 = do_mysql_fetch_assoc($res7))
          ...
          if ($data3 = do_mysql_fetch_assoc($res3)) {
            ...
            if (($data4= do_mysql_fetch_assoc($res4)) && ($data5 = do_mysql_fetch_assoc($res5))) {
  ...
  function neutral($pid) {
    ...
    if (($this->player != $pid) && ($data1=do_mysql_fetch_assoc($res1))) {
  ...
  function hostile($pid) {
    ...
    if (($this->player != $pid) && ($data1=do_mysql_fetch_assoc($res1))) {
      ...
      $this->message($this->player, "Kriegserklärung an ".$data1['name']." fehlgeschlagen", "Um ".$data1['name']." den Krieg erklären zu können müssen sich folgende Truppen zurückziehen:\n\n".$msg);
        ...
        while($cit = do_mysql_fetch_assoc($resA)) {
  ...
  function offerBND($pid) {
    ...
    if (($data1 = do_mysql_fetch_assoc($res1)) && ($this->religion == $data1['religion'])) {
