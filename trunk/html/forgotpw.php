...
if ($getkey) {
  $sql_login = do_mysql_query("SELECT id,email,register_email,status FROM player ".
                                " WHERE login = '".mysql_escape_string($loginname)."'");
...
elseif ($changepw) {
	$sql_login = do_mysql_query("SELECT id,activationkey FROM player WHERE login = '".mysql_escape_string($pname)."'");
	if (mysql_num_rows($sql_login)>0) {
		$db_login = mysql_fetch_assoc($sql_login);
			...
			if (checkPassword($pw1)) {
				if ($pw1==$pw2) {
				    if($db_login['activationkey'] == null || strlen($db_login['activationkey']) == 0) {
					  ...
					  do_mysql_query("UPDATE player SET activationkey=NULL, password='".md5(trim($pw1))."' WHERE id=".$db_login['id']) or die(mysql_error());
