...
function attMSG($end, $endtime, $attowner, $defenders, $defowner, $erg, $attstr, $defstr, $playerkilled, $cityName) {
  ...
  for ($i = 0; $i < sizeof($erg); $i ++) {
    ...
    if($erg[$i]['player']) {
      ...
      $uownerName = do_mysql_fetch_assoc($uowner);
    }
    else {
...
function goBack($armyid, $reason) {
  ...
  if ($data1['mission'] == "return" || $data1['end'] == $data1['start']) {
    ...
    $data7 = do_mysql_fetch_assoc($res7);
    if ($data7 != FALSE && ($data7['owner'] == $data1['owner'] || getWarRel($data7['owner'], $data1['owner']) != 1)) {
      ...
      if ($pos = do_mysql_fetch_assoc($pos_res)) {
        ...
        if ($armyunits = do_mysql_fetch_assoc($armyunits_res)) {
  ...
  if ($reason != 0) {
    ...
    $pos = do_mysql_fetch_assoc($pos_res);
...
function arrivalArmy() {
  ...
  while ($data1 = do_mysql_fetch_assoc($res1)) {
    ...
    //Stadt vorhanden?
    if ($data7 = do_mysql_fetch_assoc($res7)) {
      if (DEBUG_SERVICE)
...
function getDefenderIDs($cityid, $cityowner) {
  ...
  if($cityowner) {
    ...
    while ($defender = do_mysql_fetch_assoc($defenders_res)) {
...
function arrivalAttack($mission, $cityid, $cityowner, $cityname, $citypopulation, $armyid, $armyowner, $endtime, $tactic) {
  ...
  //Angreifer gewonnen
    ...
    if (DEBUG_SERVICE) {
      ...
      $t = 0;
      while ($att_troops = do_mysql_fetch_assoc($get_att_troops)) {
    ...
    elseif ($mission == "burndown") {
      ...
      if ($burndata = do_mysql_fetch_assoc($burnres))
      ...
      if ($burnerg[0]['player'] == $armyowner) {
        ...
        if ($price['gold'] > 0) {
          $attstr .= "Einige verrichteten diese Aufgabe mit Freude und raubten alles, ".
            ...
            "letzte Einwohner getätet oder vertrieben war.";
...
function updateRes() {
  ...
  echo " Tick = ".$tick.", Upt = '".date("d.m.y G:i:s", $upt)."' ($upt), Datensätze: ".mysqli_num_rows($res1)."\n";
