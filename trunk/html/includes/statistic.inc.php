...
function getTimer($id) {
  ...
  if ($last=mysql_fetch_assoc($last_res)) {
...
<h3>Legende:</h3>
<b>Tick:</b> Abrechnungsintervall (alle <? echo TICK/60; ?> Minuten)<br>
<b>EW:</b> Einwohner<br>
...
<?
  ...
  while($c = mysql_fetch_assoc($cities)) {
    $citysum = 0;
...
function stat_weapons($id) {
  ...
  while($data2=mysql_fetch_assoc($res2)) {
    ...
    $res4 = do_mysql_query("SELECT sum(count * res_storage) AS storagelimit FROM building LEFT JOIN citybuilding ON citybuilding.building = building.id WHERE city = ".$data2['id']) or die(mysql_error());
...
function stat_building_ordered($id, $sort) {
  ...
  $build = array();
  while ($buildings = mysql_fetch_assoc($buildings_res)) {
...
function stat_troop_ordered($id,$sort) {
  ...
  $troop = array();
  while ($troops = mysql_fetch_assoc($troops_res)) {
...
function stat_troop_army($id) {
  ...
  if (mysql_num_rows($armies_res)==0) {
  ...
  while ($armies=mysql_fetch_assoc($armies_res)) {
...
function stat_troop_enemy($id) {
  ...
  if(mysql_num_rows($spy_res)>0) {
    while($spy=mysql_fetch_assoc($spy_res)) {
      ...
      echo '<td valign="top">';
      while($spy_unit=mysql_fetch_assoc($spy_unit_res)) {
...
function stat_troopoverview($id) {
  ...
  while( $unit = mysql_fetch_array($units1)) {
    $unitssum += $unit['cnt'];
  ...
  $unitssum = 0;
  while( $unit = mysql_fetch_array($units2)) {
    $unitssum += $unit['cnt'];
