...
if ( isset($HTTP_POST_VARS['cancel']) )
{
	if ( $post_id )
	{
		$redirect = "cf_viewtopic.$phpEx?" . POST_POST_URL . "=$post_id";
		$post_append = "#$post_id";
	}
	else if ( $topic_id )
	{
...
if ( $result = $db->sql_query($sql) )
{
	...
	if ( $mode == 'editpost' || $mode == 'delete' || $mode == 'poll_delete' )
	{
		...
		if ( $post_info['poster_id'] != $userdata['user_id'] && !$is_auth['auth_mod'] )
		{
			...
			$message .= '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . append_sid("cf_viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
...
if ( !$is_auth[$is_auth_type] )
{
	...
	redirect(append_sid("login.$phpEx?redirect=cf_posting.$phpEx&" . $redirect, true));
}
...
if ( ( $delete || $poll_delete || $mode == 'delete' ) && !$confirm )
{
	...
	$template->assign_vars(array(
		...
		'S_CONFIRM_ACTION' => append_sid("cf_posting.$phpEx"),
...
else if ( $mode == 'vote' )
{
	...
	if ( !empty($HTTP_POST_VARS['vote_id']) )
	{
		...
		$message .=  '<br /><br />' . sprintf($lang['Click_view_message'], '<a href="' . append_sid("cf_viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id") . '">', '</a>');
		message_die(GENERAL_MESSAGE, $message);
	}
	else
	{
...
$is_mod=false;
$res1=mysql_query("SELECT group_id FROM clanf_user_group WHERE user_id='".$_SESSION['player']->id."'");
...
$template->assign_vars(array(
	...
	'U_VIEWTOPIC' => ( $mode == 'reply' ) ? append_sid("cf_viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;postorder=desc") : '', 
	'U_REVIEW_TOPIC' => ( $mode == 'reply' ) ? append_sid("cf_posting.$phpEx?mode=topicreview&amp;" . POST_TOPIC_URL . "=$topic_id") : '', 
	...
	'S_TOPIC_ID' => $topic_id, 
	'S_POST_ACTION' => append_sid("cf_posting.$phpEx"),
