...
if ( isset($HTTP_POST_VARS['cancel']) )
{
	if ( $topic_id )
	{
		$redirect = "cf_viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id";
	}
	else if ( $forum_id )
	{
		$redirect = "cf_viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id";
	}
	else
	{
...
switch( $mode )
{
	case 'delete':
		...
		if ( $confirm )
		{
			...
			if ( !empty($topic_id) )
			{
				$redirect_page = "cf_viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'];
				$l_redirect = sprintf($lang['Click_return_forum'], '<a href="' . $redirect_page . '">', '</a>');
			}
			else
			{
				$redirect_page = "cf_modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'];
		...
		else
		{
			...
			$template->assign_vars(array(
				...
				'S_CONFIRM_ACTION' => append_sid("cf_modcp.$phpEx"),
	...
	case 'move':
		...
		if ( $confirm )
		{
			...
			if ( !empty($topic_id) )
			{
				$redirect_page = "cf_viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'];
			...
			$message = $message . '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . "cf_viewforum.$phpEx?" . POST_FORUM_URL . "=$old_forum_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
		...
		else
		{
			...
			$template->assign_vars(array(
				...
				'S_MODCP_ACTION' => append_sid("cf_modcp.$phpEx"),
				'S_HIDDEN_FIELDS' => $hidden_fields)
			);
	...
	case 'lock':
		...
		$message = $message . '<br \><br \>' . sprintf($lang['Click_return_forum'], '<a href="' . "cf_viewforum.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
	...
	case 'split':
		...
		if ($post_id_sql != '')
		{
			...
			if ($row = $db->sql_fetchrow($result))
			{
				...
				$template->assign_vars(array(
					'META' => '<meta http-equiv="refresh" content="3;url=' . "cf_viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'] . '">')
				);

				$message = $lang['Topic_split'] . '<br /><br />' . sprintf($lang['Click_return_topic'], '<a href="' . "cf_viewtopic.$phpEx?" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'] . '">', '</a>');
				message_die(GENERAL_MESSAGE, $message);
			}
		}
		else
		{
			...
			if( ( $total_posts = $db->sql_numrows($result) ) > 0 )
			{
				...
				$template->assign_vars(array(
					...
					'S_SPLIT_ACTION' => append_sid("cf_modcp.$phpEx"),
	...
	case 'ip':
		...
		$template->assign_vars(array(
			...
			'U_LOOKUP_IP' => "cf_modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . "=$post_id&amp;" . POST_TOPIC_URL . "=$topic_id&amp;rdns=$ip_this_post&amp;sid=" . $userdata['session_id'])
		);
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				$template->assign_block_vars('iprow', array(
					...
					'U_LOOKUP_IP' => "cf_modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . "=$post_id&amp;" . POST_TOPIC_URL . "=$topic_id&amp;rdns=" . $row['poster_ip'] . "&amp;sid=" . $userdata['session_id'])
				);
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				$template->assign_block_vars('userrow', array(
					...
					'U_PROFILE' => ($id == ANONYMOUS) ? "cf_modcp.$phpEx?mode=ip&amp;" . POST_POST_URL . "=" . $post_id . "&amp;" . POST_TOPIC_URL . "=" . $topic_id . "&amp;sid=" . $userdata['session_id'] : append_sid("profile.$phpEx?mode=viewprofile&amp;" . POST_USERS_URL . "=$id"),
					'U_SEARCHPOSTS' => append_sid("cf_search.$phpEx?search_author=" . urlencode($username) . "&amp;showresults=topics"))
				);
	...
	default:
		...
		while ( $row = $db->sql_fetchrow($result) )
		{
			...
			$u_view_topic = "cf_modcp.$phpEx?mode=split&amp;" . POST_TOPIC_URL . "=$topic_id&amp;sid=" . $userdata['session_id'];
		...
		$template->assign_vars(array(
			'PAGINATION' => generate_pagination("cf_modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'], $forum_topics, $board_config['topics_per_page'], $start),
